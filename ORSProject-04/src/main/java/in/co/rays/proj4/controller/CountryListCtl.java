...
@WebServlet(name = "CountryListCtl", urlPatterns = { "/ctl/CountryListCtl" })
public class CountryListCtl extends BaseCtl {
	...
	protected void preload(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			List CountryList = CountryModel.list();
			 System.out.println("asdfghjkl;"+CountryList);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		CountryBean bean = (
				CountryBean) populateBean(request);
		CountryModel model = new CountryModel();

		try {
			List<CountryBean> list = model.search(bean, pageNo, pageSize);
			List<CountryBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					CountryBean deletebean = new CountryBean();
					for (String id : ids) {
	...
	protected String getView() {
		return ORSView.COUNTRY_LIST_VIEW;
	}
}