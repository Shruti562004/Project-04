...
@WebServlet(name = "TaxListCtl", urlPatterns = { "/ctl/TaxListCtl" })
public class TaxListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<TaxBean> list = model.search(bean, pageNo, pageSize);
			List<TaxBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					TaxBean deletebean = new TaxBean();
					for (String id : ids) {
