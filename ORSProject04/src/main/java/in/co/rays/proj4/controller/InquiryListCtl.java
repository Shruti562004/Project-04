...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	try {
		List<InquiryBean> list=model.search(bean, pageNo, pageSize);
		List<InquiryBean>  next=model.search(bean, pageNo+1, pageSize);
...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	List<InquiryBean> list = null;
	List<InquiryBean> next = null;
	...
	try {
		...
		if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
			...
			if (ids != null && ids.length > 0) {
				InquiryBean deletebean = new InquiryBean();
				for (String id : ids) {
					deletebean.setId(DataUtility.getInt(id));
					model.delete(deletebean);
					ServletUtility.setSuccessMessage("inquiry deleted successfully", request);
				}
			} else {
