...
@WebServlet("/AnnounceListCtl")
public class AnnounceListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			List<AnnounceBean> list = model.search(bean, pageNo, pageSize);
			List<AnnounceBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					AnnounceBean deletebean = new AnnounceBean();
					for (String id : ids) {
	...
	protected String getView() {
		return ORSView.ANNOUNCE_LIST_VIEW;
	}
}